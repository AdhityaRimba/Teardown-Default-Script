...
function init()
	...
	pos = GetLightPos(light)
	setLightEnabled(false)
	
	if pType == 1 then snd = LoadSound("alarm1") end
	if pType == 2 then loop = LoadLoop("alarm2-loop") end
	if pType == 3 then loop = LoadLoop("alarm3-loop") end
...
function update(dt)
	...
	if GetBool("level.alarm") then
		...
		if pType == 4 then
			...
			joints = GetJointsConnectedToShape(shape)
