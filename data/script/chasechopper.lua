...
function init()
	...
	chopperSound = LoadLoop("chopper-loop")
	chopperChargeSound = LoadSound("chopper-charge")
	chopperShootSound = LoadSound("chopper-shoot")
	chopperRocketSound = LoadSound("tools/launcher0.ogg")
	
	chopperStartSound = LoadSound("chopper-start")
	chopperEndSound = LoadSound("chopper-end")
	chopperSoundSound = LoadSound("chopper-sound")
...
function canSeePlayer()
	...
	if v ~= 0 then RaycastRejectVehicle(v) end
	RaycastRejectTransparent()
	RaycastRejectBody(chopper)
	return not Raycast(chopperTransform.pos, dir, dist)
end
...
function getSoundVolume(pos)
	...
	local origin = VecAdd(chopperTransform.pos, VecScale(dir, 5))
	local dist = d - 5 - 2
	local blockedFactor = 1.0
	if Raycast(origin, dir, dist) then
		blockedFactor = 0.5
	end
...
function computeSurroundingHeight()
	RaycastRejectBody(chopper)
	RaycastRejectBody(mainRotor)
	RaycastRejectBody(tailRotor)
	RaycastRejectBody(searchLight)
	local probe = Vec(chopperTargetPos)
	probe[1] = probe[1] + math.random(-10, 10)
	probe[2] = 100
	probe[3] = probe[3] + math.random(-10, 10)
	local hit, dist = Raycast(probe, Vec(0,-1,0), 100, 2.0)
	local hitHeight = 0
	if hit then
		hitHeight = 100 - dist
	end
