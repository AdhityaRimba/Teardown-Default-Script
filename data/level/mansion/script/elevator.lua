...
function press(shape)
	PlaySound(clickDown,GetShapeWorldTransform(shape).pos)
...
function unpress(shape)
	PlaySound(clickUp,GetShapeWorldTransform(shape).pos)
	local t = GetShapeLocalTransform(shape)
	t.pos[1] = t.pos[1] - 0.05
	SetShapeLocalTransform(shape, t)
	if downPressed then
...
function tick(dt)
	...
	for i=1,#down do
		if IsShapeOperated(down[i]) and not upPressed then
			upTimer = 0
			downTimer = pTimer
	...
	for i=1,#up do
		if IsShapeOperated(up[i]) and not downPressed then
			downTimer = 0
			upTimer = pTimer
